...
function detectLatestAudit(wb) {
  ...
  // R-FAILLOUD: no audit sheet at all → return null (caller throws). Keeping a
  // null return (rather than throwing here) keeps detectLatestAudit pure.
...
function parseAudit(wb, state) {
  ...
  // back to a stale anchor (which would emit silently-wrong stock).
  if (!detected) {
    throw new Error(
 ...
 *
 * D2 (FIX-SPEC): the central warehouse has NO cutoff. This engine ALWAYS uses
 * the latest detected audit as the baseline + every production / daily-movement
 * row dated STRICTLY AFTER the audit date (no upper bound). Any `opts.dateCutoff`
 * a caller might pass (used for the marketplace sheets) is intentionally IGNORED
 * here — see computeBalances(), which only filters `<= anchor`, never an upper
 * bound.
 *
 * @param {File|Blob} file  uploaded workbook (browser File/Blob with arrayBuffer())
 * @param {object} [opts]   reserved; `opts.dateCutoff` is intentionally ignored (D2)
 * @returns {Promise<object>} { anchorDate, anchorSheet, asOf, config,
 *                              nonConstraining, fg, components, fixedAssets,
 *                              consumables, flags }
 * @throws {Error} R-FAILLOUD: no Audit tab / missing sections / zero FG rows.
 */
export async function parseCentralWhWorkbook(file, opts = {}) { // eslint-disable-line no-unused-vars
  ...
  return {
    ...
    // D1: components that can never be the producible bottleneck (cartons +
