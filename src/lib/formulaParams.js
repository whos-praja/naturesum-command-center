...
export const FORMULA_REFERENCE = [
  ...
  },
  {
    ...
    plain: "Days the central-WH finished goods last at the company's TOTAL SALES velocity (sum of per-channel sales rates), NOT warehouse movement — bulk replenishment shipments out of the WH are not consumption. Null (and red) when sellable stock (FG + producible) ≤ 0; null when there's stock but no sales signal.",
    where: "src/data.js · centralWhRunway / runway",
  },
  {
    id: "runwayStatus",
    name: "Runway status chip",
    expression: "status = sellable(FG+producible) ≤ 0 → red | runway ≤ leadTime → red | runway < amberRunwayDays → amber | else green",
    plain: "Tri-state color on every row. Red = stocked out (no sellable units) OR will run out before the replenishment lead time. Amber = runway below the comfort buffer (amberRunwayDays). Green = healthy. (A SKU with stock but no sales signal shows amber.)",
    where: "src/data.js · runwayStatus",
  },
  {
    id: "velocity",
    name: "Central-WH velocity",
    expression: "centralWhVelocity = channelSalesVel.amazon + channelSalesVel.flipkart + channelSalesVel.blinkit",
    plain: "Sum of the per-channel SALES velocities (Amazon leg folds website D2C per R-FBA). This is the rate the WH buffer actually drains, because it ultimately supplies every channel. It is NOT warehouse stock movement — the engine's FG depletion is dominated by bulk shipments to marketplace WHs and is no longer used for velocity / runway / growth.",
    where: "src/data.js · centralWhVelocity (= totalSalesVel)",
  },
  {
    id: "amazonChannelVel",
  ...
  },
  {
    ...
    expression: "growth = max( monthOverMonth rates )    (up to 3 MoMs from up to 4 months of sales), clamp ±200",
    plain: "Forward growth = the MAX of the trailing month-over-month SALES growth rates (up to 3, computed from up to 4 months) — plan for the highest growth seen so you don't under-stock. Fewer months → fewer MoMs; <2 months → 0. Each channel uses its OWN sales source. Clamped to ±200% so a near-zero base can't make the ratio explode.",
  ...
  },
  {
    id: "stockValue",
    name: "Stock value",
    expression: "stockValue = centralWhFG × sellingPrice    ·    headline Total = Σ over all locations (units × SP)",
    plain: "Per-SKU value = central-WH finished goods × selling price (SP, not MRP/cost). The headline Total stock value at the top of Inventory sums units × SP across EVERY location (WH + Amazon + Flipkart + Blinkit), each physical unit counted exactly once — no double-count, no omission.",
    where: "src/data.js · stockValue · PageInventory total",
  },
  {
    id: "cascadeRunway",
